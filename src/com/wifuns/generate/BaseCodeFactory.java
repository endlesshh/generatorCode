...
public abstract class BaseCodeFactory {
	 ...
	 * @throws IOException
	 */
	public void generateFile(String templateFileName, String type, Map<String,Object> data) throws TemplateException, IOException {
		try {
			String entityPackage = data.get(Globals.entityPackage).toString();
	 ...
	 * @throws IOException
	 */
	public void generateHtml2File(String html, String type,Map<String,Object> data) throws TemplateException, IOException {
		try {
			String entityPackage = data.get(Globals.entityPackage).toString();
			String entityName = data.get(Globals.entityName).toString();
			String fileNamePath = getCodePath(type, entityPackage, entityName);
	...
	public void invoke(List<ExecuteModel> executes) throws TemplateException, IOException {
		if(executes != null && executes.size()>0){
			...
			for(ExecuteModel exe : executes){ 
				generateFile(exe.getTemplateName(),exe.getStyle(), data);
			} 	
		} 
	}
	...
	public void invokeNotFlt(List<ExecuteModel> executes) throws TemplateException, IOException {
		if(executes != null && executes.size()>0){
			...
			data = this.generator.execute();
			for(ExecuteModel exe : executes){ 
				generateHtml2File(exe.getTemplateName(),exe.getStyle(), data);
			} 	
		} 
	}
